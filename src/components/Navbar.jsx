  ...
  return (
    <nav className="fixed top-0 w-full bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md z-50 border-b border-zinc-200 dark:border-zinc-800 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden md:flex gap-8 items-center">
            {navLinks.map(({ label, id }) => (
              <button
                ...
                className="font-sans text-sm text-zinc-500 dark:text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition"
              >
                {label}
              </button>
            ))}
        ...
        {/* Mobile menu */}
        {isOpen && (
          <div className="md:hidden py-3 border-t border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-950">
            {navLinks.map(({ label, id }) => (
              <button
                key={id}
                onClick={() => scrollToSection(id)}
                className="block w-full text-left px-2 py-2.5 font-sans text-sm text-zinc-600 dark:text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition"
              >
                {label}
              </button>
            ))}
